...
```{r}
test_data$target <- NA
merged_data <- rbind(train_data, test_data) #get the sample of each dataset
...
log_mod <- h2o.glm(x = colnames(train.h2o)[-1], 
                   y = "target", 
                   training_frame = train.h2o,
                   family = "binomial",
                   interactions = colnames(train.h2o)[-1],
                   standardize = TRUE,
                   lambda_search = TRUE)
```
...
forest_mod <- h2o.randomForest(x = colnames(train.h2o)[-1], y = "target", 
               training_frame=train.h2o, ntrees = 100,
                          max_depth = 30,
                          nbins_cats = 1115)
```
...
fwrite(submission_data, "forest_model.csv", row.names = FALSE)
```
