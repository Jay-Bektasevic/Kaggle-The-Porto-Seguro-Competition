...
train_data <- fread("train.csv") # <- Replace the file to train.csv
test_data <- fread("test.csv")
seguro_train[seguro_train == -1] <- NA
...
#test.h2o <- as.h2o(test_data[,-c(1,2)])
```
...
```{r warning = FALSE, message = FALSE, results = "hide"}
pr <- prediction(pred, newtrain[,"target"])
...
head(probabilities)
pr <- prediction(probabilities[,1], train_data[,"target"])
prf <- performance(pr, measure = "tpr", x.measure = "fpr")
```
...
(auc <- auc@y.values[[1]]) 
```
Not great AUC score we want it closer to the 1 then .5 unfortuntely ours is coloser to .5, so we'll have to do some feature engineering.


#CART Modeling

Make sure all the categorical variables are converted into factors. The function **rpart** will run a regression tree if the response variable is numeric, and a classification tree if it is a factor. 

Step1: Begin with a small cp.
```{r}
tree_model <- rpart(target ~ ., data = train_data, control = rpart.control(cp = 0.0000001))   
```
...
```{r}
bestcp <-  which.min(tree_model$cptable[,"xerror"])
cp <- tree_model$cptable[bestcp, "CP"]
```
Step3: Prune the tree using the best cp.
```{r}
tree_pruned <- prune(tree_model, cp )
```

```{r}

mean(test_data$target == predict(tree_model,  test_data, type="class"))
table(pred=predict(tree_model,  test_data, type="class"),true=test_data$target)
```
```{r}
