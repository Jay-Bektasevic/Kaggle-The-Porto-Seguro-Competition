...
For this feat, we will use **liquidSVM** package. 
...
model <- svm(target ~ ., data=train_data, kernel="linear", probability=TRUE)
pred_svm <- predict(model, train_data, probability=TRUE)
head(attr(pred_svm, "probabilities"))
```
...
```{r}
results <- test(svm_model, test_data)
errors(results)
```
```
