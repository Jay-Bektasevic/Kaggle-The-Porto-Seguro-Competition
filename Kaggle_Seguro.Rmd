...
```{r}
test_data$target <- NA
merged_data <- rbind(train_data[1:10000,], test_data[1:10000,]) #get the sample of each dataset
...
merged_data[, ps_ind_bin_sum := ps_ind_06_bin +  ps_ind_07_bin + ps_ind_08_bin + ps_ind_09_bin + ps_ind_10_bin + ps_ind_11_bin +
...
```{r}
nobs <- nrow(merged_data_train) 
merged_data_train <- merged_data[1:10000,]
train_inx <- sample(nrow(merged_data_train), 0.8 * nobs)

train_data <- merged_data_train[train_inx,]
test_data <- merged_data_train[-train_inx,]
...
```{r}
newtrain <- encode_fun(df = data.frame(train_data[,-1]))
...
When submitted this model was given score of 0.259.
