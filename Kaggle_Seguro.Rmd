...
By, balancing classes it appears that our AUC score incheds up but our gini index actually falls down to **0.269**. 
by adding weights and 10-fold cross-validation on training data it bumps our score to **0.273**. Note that at this point, GBM model does not allow for concurent parameters of **weights_column** and **balance_classes**. We need to find a different way of ballancing classes through upsampling/downsampling.
...
we will now use all the important varibles in the model and rerun the model using only the variables that we've selected
```{r}
...
fwrite(submission_data, "gbm_model_bal2.csv", row.names = FALSE)
```
